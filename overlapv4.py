...
# Load JSON data
with open('unc_major_requirements.json', 'r') as file:
    jsonData = json.load(file)

# Load focus capacity data
with open('unc_focus_capacities.json', 'r') as file:
...
available_majors = {major["major"]: major for major in jsonData if "major" in major and "requirements" in major}
...
app.layout = html.Div([
    ...
    dcc.Dropdown(id="majors-dropdown", options=major_options, placeholder="Select majors",
...
], style={"backgroundColor": "black", "color": "white", "fontFamily": "Arial, sans-serif", "minHeight": "100vh", "padding": "20px"})
...
def update_heatmap(selected_majors):
    ...
    fig.update_layout(
        title=f'Course Overlap Between Selected Majors',
    ...
    for i, major1 in enumerate(selected_majors):
        for j, major2 in enumerate(selected_majors):
            if i < j:  # Avoid duplicate calculations (symmetrical matrix)
                overlap_courses = list(set(req["code"].strip() for req in available_majors[major1]["requirements"] if "code" in req)
                                      .intersection(set(req["code"].strip() for req in available_majors[major2]["requirements"] if "code" in req)))
                overlap_text.append(html.Li(f"Overlap between {major1} and {major2}: {', '.join(overlap_courses) if overlap_courses else 'No overlap'}"))
